...
$query = mysqli_query(mysqli_connect("localhost", "root", "", "ttms2"), "SELECT * FROM subjects ");
...
while ($row = mysqli_fetch_assoc($query)) {
    if ($row['course_type'] == 'LAB')
        continue;
    $temp = new Subject();
    $temp->name = $row['subject_name'];
    $temp->semester = $row['semester'];
    $temp->subjectteacher = $row['allotedto'];
    if (isset($temp->subjectteacher)) {
        $teacheralias_query = mysqli_query(mysqli_connect("localhost", "root", "", "ttms2"),
...
$query = mysqli_query(mysqli_connect("localhost", "root", "", "ttms2"), "SELECT * FROM teachers ");
...
for ($I = 0; $I < $subjects_count * 4; $I++) {
    ...
    $year = $sem;
    $classroom_query = mysqli_query(mysqli_connect("localhost", "root", "", "ttms2"),
        ...
        if (isset($subjectslots[$sem][$r % 6][$j % 5])) {
            ...
            for ($z = 0; $z < 5; $z++) {
                if (isset($subjectslots[$sem][$r % 6][$z])) {
                    ...
                    if ($subjectslots[$sem][$r % 6][$z] == $subjects[$i]->name) {
                        $already = true;
                    }
                }
            }
            ...
            // set subject
            $subjects[$i]->classes++;
            $subjectslots[$sem][$r % 6][$j % 5] = $subjects[$i]->name;
            $aliasslots[$sem][$r % 6][$j % 5][0] = $subjects[$i]->alias;
            $teachers[$tindex]->days[$sem % 2][$r % 6][$j % 5] = $subjects[$i]->name;
...
for ($I = 0; $I < 2 * $count; $I++) {
    ...
    for ($j = 0; $j < 6; $j++) {
        if (isset($subjectslots[$sem][$j][5])) {
            ...
            $subjectslots[$sem][$j][5] = $practicals[$i]->name;
            $aliasslots[$sem][$j][5][0] = $practicals[$i]->alias;
            $aliasslots[$sem][$j][5][1] = $practicals[$i]->alias2;
            $aliasslots[$sem][$j][5][2] = $practicals[$i]->alias3;
            $teachers[$tindex]->days[$sem % 2][$j][5] = $practicals[$i]->name;
            $teachers[$tindex2]->days[$sem % 2][$j][5] = $practicals[$i]->name;
            $teachers[$tindex3]->days[$sem % 2][$j][5] = $practicals[$i]->name;
            break;
        }
    }
}
...
period6='" . $subjectslots[$i][$j][5] . "<br>" . $aliasslots[$i][$j][5][0] . ", " . $aliasslots[$i][$j][5][1] . ", " . $aliasslots[$i][$j][5][2] . "'
 WHERE day='" . $days[$j] . "' ";
        $q = mysqli_query(mysqli_connect("localhost", "root", "", "ttms2"), $query);
    }
...
echo "<script>
window.location.href='generatetimetable.php;
</script>";
*/
?>
